...
@app.command()
def run(
    profiles_path: Path = typer.Option(DATA_DIR / "panel_profiles.csv", "--profiles-path"),
    ...
    Args:
        profiles_path: CSV produced by ``generate_profiles_cli.py`` (columns
            ...
            falls back to a neutral profile (logged as a warning). If the
            file itself doesn't exist, every panel gets a neutral profile
            (equivalent to ``--mode uniform``) -- generating profiles first
            is recommended (see the module README) but not required to run
            this at all.
    ...
    if profiles_path.exists():
        ...
        logger.warning(
            f"{profiles_path} not found; using a neutral profile (factor=1.0, offset=0.0) "
            f"for every panel -- run generate_profiles_cli.py first to customize this"
        )
        profiles = generate_profiles(list(panels.keys()), mode="uniform").set_index("panel_tag")
